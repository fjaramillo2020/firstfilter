terraform {
  ...
  backend "azurerm" {
    ...
    key                  = "common/state/terraform.state"
  }
...
# ResourceGroup
resource "azurerm_resource_group" "id_state" {
  name     = var.id_state_rg.name
  location = var.id_state_region

  tags = merge(var.env_tag, var.id_state_rg_tags)
}
